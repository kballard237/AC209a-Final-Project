...
In order to identify if there is a potential correlation between motion and light, we first inverted light so that 0 represents if light is off and 4095 represents light on. We then scaled light by a factor of 10 to overlay it with motion count. We then scaled pressure by a factor of 10000 to overlay it as well. We group each variable by hour and visualize each by weekday. 
